...
Use `assets/GAME-BRIEF.template.md`, `assets/ASSET-PIPELINE.template.md`, `assets/ACCEPTANCE.template.md`, and `assets/QA-ROUTE.template.md` directly. Resolve the script from this skill’s own directory, not from the game project, then run:
...
3. complete player-like route from gaining control through failure/restart and ending;
...
8. the same route on the production deployment when release is in scope.

Keep `docs/ACCEPTANCE.md` current. A pass needs fresh evidence from the exact build and route. Mark missing evidence `not-run`, not pass. Do not accept solely from headless tests, static screenshots, counters, or agent status.
