...
- A demo becomes a prototype as soon as the work adds a goal, score, failure or rules. A new game request that asks in its own words for release, polish, AAA or console quality, or a vertical slice, with no size word, starts in production. An existing prototype moves to production only when the user says "production" or clearly agrees to the switch; "Moving up" in `references/demo-prototype-lanes.md` says when to propose it and how to switch.
...
- the latest stable Three.js release available when a new project, a major rebuild or a Build/change of an existing project begins, resolved from official sources and then pinned exactly in that project's lockfile (or its import-map URLs) and its evidence, rather than frozen in this reusable skill;
...
Use the actual development host as a disclosed proxy only when the intended target is unknown or unavailable; never invent a device or call the proxy a target pass. Mark final-target checks `needs-user` once their check card, with the exact route and capture steps, has been handed to the user or a lab machine (see "Report evidence honestly"); until then they stay `not-run`.
...
Open <URL> in its own desktop Chrome or Edge tab with WebGPU, not the preview pane. Build <id>.
...
Production cards leave the frame-rate line off: the frame-time and deployment-route cards follow Gates 5 and 7 in `references/verification.md`, and a frame-time reply without a trace or numbers leaves that gate `needs-user`.
